...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!email) {
      return NextResponse.json({ success: false, error: 'email 必填' }, { status: 400 })
    }
    ...
    // 标记邮箱已确认
    // @ts-ignore - supabase-js 支持 email_confirm 短字段
    const { error: updErr } = await admin.auth.admin.updateUserById(userRow.id, { email_confirm: true })
    if (updErr) {
